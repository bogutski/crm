...
const DICTIONARIES = [
  ...
  {
    code: 'opportunity_sources',
    name: 'Opportunity Sources',
    ...
    items: [
      ...
      { code: 'email_campaign', name: 'Email Campaign', properties: { color: '#eab308' } },
      { code: 'webinar', name: 'Webinar', properties: { color: '#14b8a6' } },
      { code: 'partner', name: 'Partner', properties: { color: '#8b5cf6' } },
    ],
  },
  {
...
function generateFullAddress(country: string = 'US') {
  ...
  if (country === 'CA') {
    state = randomElement(CA_PROVINCES);
    zip = faker.location.zipCode('?#? #?#').toUpperCase();
  } else {
    state = randomElement(US_STATES);
    zip = faker.location.zipCode('#####');
  }

  return {
    line1: `${streetNumber} ${streetName}`,
    line2: Math.random() > 0.7 ? `Suite ${faker.number.int({ min: 100, max: 999 })}` : undefined,
    city,
    state,
    zip,
    country,
...
function generatePartialAddress(country: string = 'US') {
  const rand = Math.random();

  // Different variations of partial addresses
  if (rand < 0.25) {
    // Only city and state
    return {
      city: faker.location.city(),
      state: country === 'CA' ? randomElement(CA_PROVINCES) : randomElement(US_STATES),
      country,
      isPrimary: true,
    };
  } else if (rand < 0.5) {
    ...
    return {
      city: faker.location.city(),
      state: country === 'CA' ? randomElement(CA_PROVINCES) : randomElement(US_STATES),
      zip: country === 'CA' ? faker.location.zipCode('?#? #?#').toUpperCase() : faker.location.zipCode('#####'),
      country,
      isPrimary: true,
    };
  } else if (rand < 0.75) {
    // Only state and country
    return {
      state: country === 'CA' ? randomElement(CA_PROVINCES) : randomElement(US_STATES),
      country,
      isPrimary: true,
    ...
    // Line1 and city only
    return {
      line1: `${faker.number.int({ min: 1, max: 9999 })} ${faker.location.street()}`,
      city: faker.location.city(),
      country,
      isPrimary: true,
    };
...
interface DictItemIds {
  ...
  opportunitySources: mongoose.Types.ObjectId[];
...
function generateContactDoc(
  ...
  // - 30% full address (all fields)
  // - 50% partial address (some fields)
  // - 20% no address
  const addresses = [];
  ...
  if (addressRand < 0.30) {
    // Full address
    const addressCountry = Math.random() > 0.9 ? 'CA' : 'US'; // 10% Canadian
    addresses.push(generateFullAddress(addressCountry));

    // 10% chance of having a second address
    if (Math.random() > 0.9) {
      const secondAddress = generateFullAddress(Math.random() > 0.8 ? 'CA' : 'US');
      secondAddress.isPrimary = false;
      addresses.push(secondAddress);
    }
  } else if (addressRand < 0.80) {
    // Partial address (50%)
    const addressCountry = Math.random() > 0.9 ? 'CA' : 'US';
    addresses.push(generatePartialAddress(addressCountry));
  }
  // else: 20% - no address (addresses stays empty)
...
function generateTaskDoc(
  ...
  // 20% задач будут просроченными (если не указан явный статус completed/cancelled)
  const isOverdue = options.forceOverdue ?? (
    !options.status &&
  ...
  if (isOverdue) {
    // Просроченные задачи: дата от 1 до 30 дней назад
    dueDate = faker.date.between({
      from: new Date(Date.now() - 30 * 24 * 60 * 60 * 1000),
      to: new Date(Date.now() - 1 * 24 * 60 * 60 * 1000),
    });
    // Просроченные задачи только open или in_progress
    status = options.status || randomElement(['open', 'in_progress'] as const);
  } else {
    dueDate = faker.date.between({
      from: new Date(Date.now() - 7 * 24 * 60 * 60 * 1000),
      to: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000),
    });
    status = options.status || randomElement([...taskStatuses]);
  }
...
async function main() {
  ...
  for (const dictData of DICTIONARIES) {
    ...
    for (const item of items) {
      if ('children' in item && item.children) {
        ...
        } else if (dictData.code === 'opportunity_sources') {
          dictIds.opportunitySources.push(itemId);
  ...
  // Tasks for projects (some will be overdue based on 20% probability in generateTaskDoc)
  ...
  // Standalone tasks (20% will be overdue automatically)
  ...
  // Explicitly overdue tasks with realistic titles
  const overdueTasks = [
    { title: 'Send commercial proposal', description: 'Client waiting for proposal since last week' },
    { title: 'Follow up with lead', description: 'Lead expressed interest but went cold' },
    { title: 'Sign contract', description: 'Contract review completed, awaiting signature' },
    { title: 'Pay vendor invoice', description: 'Invoice #4521 overdue for payment' },
    { title: 'Complete database audit', description: 'Security audit required by compliance' },
    { title: 'Update API documentation', description: 'New endpoints need to be documented' },
    { title: 'Configure backup system', description: 'Backup schedule needs to be set up' },
    { title: 'Interview candidate', description: 'Senior developer position - reschedule needed' },
    { title: 'Submit expense report', description: 'Q3 expenses pending approval' },
    { title: 'Review pull request', description: 'Critical bug fix waiting for code review' },
  ];
  ...
  console.log(`   Created ${COUNTS.standaloneTasks + overdueTasks.length} standalone tasks (${overdueTasksCreated} explicitly overdue)`);

  // Tasks linked to contacts (20% will be overdue automatically)
  ...
  console.log(`      - 30% full address (all fields)`);
  console.log(`      - 50% partial address (some fields)`);
  console.log(`      - 20% no address`);
  console.log(`\n  AI Integration:`);
